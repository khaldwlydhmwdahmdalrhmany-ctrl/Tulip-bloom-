...
/**
 * ⚠️ `implemented` — حارس السلامة.
 *
 * تعني: هل كُتب كود الاستدعاء الفعلي لهذه البوابة؟
 *
 * البوابة غير المنفَّذة **لا تُعرض للعميل إطلاقًا** مهما فُعّلت
 * وأُضيفت مفاتيحها. بدون هذا الحارس يكفي أن يفعّل المشغّل بوابة
 * مبكّرًا ليختار عميل «مدى» ثم يُسجَّل طلبه بلا دفع — وهو إرباك
 * لا يكشفه أحد إلا بعد شكوى.
 *
 * ⚠️ عند كتابة تكامل بوابة: اقلبها إلى `true` **بعد** اختبار
 * عملية دفع حقيقية في وضع الاختبار، لا قبله.
 */
