...
async function runMigrations(pool) {
  ...
  /* ── v15: مكتبة الوسائط ──
   * سجلّ لكل ملف مرفوع. بدونه لا سبيل لتصفّح ما رُفع سابقًا:
   * الرفع كان يُرجع رابطًا ويُنسى، فتُرفع الصورة نفسها مرارًا.
   */
  await pool.query(`
  ...
  await pool.query(`
    ...
    /* المحتوى بنفس بلوكات منشئ الصفحات — لا مُصيِّر ثانٍ ولا
       حقل HTML خام. انظر §١٨.١ لسبب رفض HTML. */
    CREATE TABLE IF NOT EXISTS posts (
      ...
      "createdAt"    TIMESTAMPTZ DEFAULT now(),
      "updatedAt"    TIMESTAMPTZ DEFAULT now(),
      CONSTRAINT posts_status_values CHECK (status IN ('draft','published'))
    );
    CREATE INDEX IF NOT EXISTS idx_posts_slug ON posts(slug);
    CREATE INDEX IF NOT EXISTS idx_posts_pub ON posts(status, "publishedAt" DESC);

    /* عناصر التنقّل — تتجاوز NAV_LINKS من التهيئة عند وجودها */
    CREATE TABLE IF NOT EXISTS nav_items (
      ...
      active      BOOLEAN DEFAULT true,
      "createdAt" TIMESTAMPTZ DEFAULT now(),
      CONSTRAINT nav_location_values CHECK (location IN ('header','footer'))
    );
  ...
  /* ── v17: الشحن والتوصيل ── */
  await pool.query(`
    /* منطقة شحن = مجموعة مدن بقواعد سعر واحدة.
       المدن نصّ مفصول بفواصل لا جدول منفصل: القائمة تُحرَّر
       ككتلة واحدة في اللوحة، وجدول علاقات هنا يضيف تعقيدًا
       بلا استعلام يبرّره. */
    ...
    CREATE TABLE IF NOT EXISTS shipping_rates (
      ...
      "minSubtotal"  DOUBLE PRECISION DEFAULT 0,
      "etaText"      TEXT,
    ...
    /* نوافذ التسليم — جوهرية في متجر ورد: الهدية لها وقت. */
    ...
    /* إعدادات شركات الشحن. المفاتيح تُحفظ هنا ولا تُرسل للمتصفح أبدًا. */
    CREATE TABLE IF NOT EXISTS carriers (
      ...
      "extraJson"     TEXT,
      "updatedAt"     TIMESTAMPTZ DEFAULT now(),
      CONSTRAINT carriers_mode_values CHECK (mode IN ('test','live'))
    );
    ...
    ALTER TABLE orders
      ...
      ADD COLUMN IF NOT EXISTS "shippingCost" DOUBLE PRECISION DEFAULT 0,
      ADD COLUMN IF NOT EXISTS "deliveryDate" TEXT,
      ADD COLUMN IF NOT EXISTS "deliverySlot" TEXT;
  `);


  /* ── v18: الدفع والمدفوعات ── */
  await pool.query(`
    CREATE TABLE IF NOT EXISTS payment_gateways (
      ...
      "sortOrder"       INTEGER DEFAULT 0,
      "updatedAt"       TIMESTAMPTZ DEFAULT now(),
      CONSTRAINT pg_mode_values CHECK (mode IN ('test','live'))
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_payments_ref ON payments("providerRef");

    /* سجلّ أحداث البوابات — أساس منع التكرار.
       البوابات تعيد إرسال الحدث حتى تتلقّى 200، وبلا مفتاح فريد
       على معرّف الحدث يُعالَج الدفع مرتين. */
    CREATE TABLE IF NOT EXISTS payment_events (
      ...
      payload     TEXT,
      "createdAt" TIMESTAMPTZ DEFAULT now(),
      CONSTRAINT uq_event UNIQUE (gateway, "eventId")
    );
...
/* ============ الطلبات ============ */
/** ⭐ مضافة لنظام الدفع — يحتاج قراءة مبلغ الطلب من القاعدة. */
