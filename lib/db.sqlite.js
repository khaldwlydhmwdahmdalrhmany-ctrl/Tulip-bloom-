...
export function getDb() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS payment_gateways (
      ...
      publishableKey TEXT, secretKey TEXT, webhookSecret TEXT, extraJson TEXT,
      sortOrder INTEGER DEFAULT 0,
      updatedAt TEXT DEFAULT (datetime('now'))
    );
    CREATE TABLE IF NOT EXISTS payments (
      id TEXT PRIMARY KEY, orderId TEXT NOT NULL, gateway TEXT NOT NULL, method TEXT,
      amount REAL NOT NULL DEFAULT 0, currency TEXT DEFAULT 'SAR',
      status TEXT DEFAULT 'pending', providerRef TEXT, redirectUrl TEXT,
      failureReason TEXT, rawJson TEXT,
      createdAt TEXT DEFAULT (datetime('now')),
      updatedAt TEXT DEFAULT (datetime('now'))
    );
    ...
    CREATE TABLE IF NOT EXISTS payment_events (
      id TEXT PRIMARY KEY, gateway TEXT NOT NULL, eventId TEXT NOT NULL,
      paymentId TEXT, type TEXT, payload TEXT,
      createdAt TEXT DEFAULT (datetime('now')),
  ...
  try { db.exec(`ALTER TABLE orders ADD COLUMN paymentStatus TEXT DEFAULT 'unpaid'`); } catch {}
  try { db.exec(`ALTER TABLE orders ADD COLUMN paymentMethod TEXT`); } catch {}
  try { db.exec(`ALTER TABLE orders ADD COLUMN paidAt TEXT`); } catch {}
...
export async function getOrderById(id) {
  return getDb().prepare(`SELECT * FROM orders WHERE id = ? LIMIT 1`).get(id) || null;
}
