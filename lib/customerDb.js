...
export async function listFavoriteIds(customerId) {
  const rows = await all(
    `SELECT ${q("productId")} AS pid FROM customer_favorites WHERE ${q("customerId")} = ? ORDER BY ${q("createdAt")} DESC`,
    [customerId]
  );
...
/**
 * تبديل المفضّلة — يُرجع الحالة الجديدة.
 * ⚠️ يعتمد على القيد الفريد في القاعدة لا على فحص مسبق:
 * الضغط المزدوج السريع يُنتج طلبين متوازيين، والفحص في الكود
 * وحده يسمح بمرورهما معًا فيُدرَج السطر مرتين.
 */
export async function toggleFavorite(customerId, productId) {
  const existing = await get(
    `SELECT id FROM customer_favorites WHERE ${q("customerId")} = ? AND ${q("productId")} = ? LIMIT 1`,
    [customerId, productId]
  );
  if (existing) {
    await run(`DELETE FROM customer_favorites WHERE id = ?`, [existing.id]);
    return false;
  }
  try {
    await run(
      `INSERT INTO customer_favorites (id, ${q("customerId")}, ${q("productId")}) VALUES (?, ?, ?)`,
      [newId(), customerId, productId]
    );
  } catch {
    // القيد الفريد رفض التكرار — الحالة المطلوبة محقّقة أصلًا
  }
  return true;
}
...
export async function removeFavorite(customerId, productId) {
  await run(
    `DELETE FROM customer_favorites WHERE ${q("customerId")} = ? AND ${q("productId")} = ?`,
    [customerId, productId]
  );
}

/** أكثر المنتجات إضافةً للمفضّلة — مؤشّر طلب مبكّر للوحة التحكم. */
export async function topFavoritedProducts(limit = 10) {
  return all(
    `SELECT ${q("productId")} AS pid, COUNT(*) AS n
